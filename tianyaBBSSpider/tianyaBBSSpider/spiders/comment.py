...
def get_comment_item(response):
    ...
    try:
        ...
        comment_divs = sel.xpath('//ul[@class="articlecomments-list"]/li')
        for comment_sel in comment_divs:
            comment_item = CommentItem()

            author_name = comment_sel.xpath('./div[@class="comments-body"]/div[@class="comments-header"]'
                                            '/span/a[contains(@class, "replyName")]/text()').extract_first()
            comment_item['author_name'] = check_value(author_name)

            author_href = comment_sel.xpath('./div[@class="comments-body"]/div[@class="comments-header"]'
                                            '/span/a[contains(@class, "replyName")]/@href').extract_first()
            comment_item['author_href'] = check_value(author_href)

            author_id = get_author_id_by_href(author_href)
            comment_item['author_id'] = check_value(author_id)

            comment_id = comment_sel.xpath('./div[@class="comments-body"]/@cid').extract_first()
            comment_item['comment_id'] = check_value(comment_id)

            content = comment_sel.xpath('./div[@class="comments-body"]/div[@class="comments-content"]').xpath('string(.)').extract_first()
            comment_item['content'] = check_value(content)

            date_time = comment_sel.xpath('./div[contains(@class, "comments-body")]/div[@class="comments-header"]'
                                          '/span[contains(@class, "reply-time")]/text()').extract_first()
            comment_item['date_time'] = check_value(date_time)
