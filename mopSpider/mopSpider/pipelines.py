...
import pymysql
from mopSpider.items import AuthorItem,CommentItem,FansItem,PostItem
# from thrift.protocol import TCompactProtocol
# from thrift.transport import TSocket
#
# from mopSpider.items import PostItem, CommentItem, AuthorItem, FansItem
# from mopSpider.utils.gen_db_put import gen_start_spider_info, gen_stop_spider_info, gen_author_put, gen_post_put, \
...
class ImagePipeline(ImagesPipeline):
    ...
    def get_media_requests(self, item, info):
        if isinstance(item, PostItem) and 'picture_hrefs' in item and item['picture_hrefs']:
            picture_urls = item['picture_hrefs'].split('|')
            for picture_url in picture_urls:
                yield Request(picture_url)
        elif isinstance(item, CommentItem) and 'picture_hrefs' in item and item['picture_hrefs']:
            picture_urls = item['picture_hrefs'].split('|')
            for picture_url in picture_urls:
                yield Request(picture_url)
