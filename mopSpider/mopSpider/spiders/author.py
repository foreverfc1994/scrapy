...
def get_author_item(response):
    ...
    num_div = sel.xpath('//*[@id="hi_user_info"]/div[2]/ul/li[4]/p[1]/a/text()').extract_first().strip()
    author_item['point'] = check_value(num_div)
    ...
    location = sel.xpath('/html/body/div[4]/div[2]/div/div[1]/div[2]/div[1]/div[2]/span[2]/text()').extract_first()
    ...
    register_date = sel.xpath('//div[@class="hpUserInfo2"]/p/span/text()').extract_first()+' 00:00:00'
    ...
    for user_info in user_info_div:
        ...
        if '生日' in user_info_name:
            if user_info_value=='无':
                author_item['birthday'] = check_value(user_info_value)
            else:
                author_item['birthday'] = check_value(user_info_value).replace(' 年 ','-').replace(' 月 ','-').replace(' 日',' ')+'00:00:00'
