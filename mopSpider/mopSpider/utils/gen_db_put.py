...
from mopSpider.utils import MD5Utils
...
def gen_comment_put(comment_item):
    ...
    for key in comment_item:
        if not key.startswith(('_', 'picture')):
            ...
            column_values.append(TColumnValue(picture_fml, (key + '_num').encode(),
                                              str(len(comment_item[key])).encode()))
            for p in range(len(comment_item[key])):
                column_values.append(
                    TColumnValue(picture_fml, (key + str(p)).encode(), str(comment_item[key][p]).encode()))
